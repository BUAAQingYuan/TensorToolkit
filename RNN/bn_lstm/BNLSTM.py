# Notice: This file is tested on TensorFlow v0.10.0 only.
...
class BNLSTMCell(RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__):
            ...
            W_xh = tf.get_variable('W_xh',[input_size, 4 * self.num_units], initializer=self.initializer)
            W_hh = tf.get_variable('W_hh',[num_proj, 4 * self.num_units],initializer=self.initializer)
            ...
            if self.use_peepholes:
                c = c_prev * tf.sigmoid(f + self.forget_bias + w_f_diag * c_prev) + tf.sigmoid(i + w_i_diag * c_prev) * self.activation(j)
            else:
                c = c_prev * tf.sigmoid(f + self.forget_bias) + tf.sigmoid(i) * self.activation(j)
